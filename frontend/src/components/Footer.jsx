    ...
    <footer className="border-t border-slate-200 bg-white">
      <div className="app-container grid grid-cols-1 gap-10 py-12 md:grid-cols-3 lg:gap-16">
        <div className="max-w-md md:pr-6">
          ...
          <p className="mt-4 text-sm leading-6 text-slate-600">
            Nền tảng học tiếng Anh trực tuyến giúp học viên theo dõi lộ trình,
            giáo viên quản lý nội dung và nhà trường kiểm soát chất lượng đào
            tạo.
          </p>
          <p className="mt-4 text-sm font-medium text-slate-700">
            Học đúng lộ trình. Theo dõi đúng tiến độ.
          </p>
        </div>

        <FooterGroup title="Khám phá" links={exploreLinks} />
        <FooterGroup
          title={user ? "Tài khoản của bạn" : "Bắt đầu"}
          links={accountLinks}
        />
      </div>
      ...
      <div className="border-t border-slate-200 bg-slate-50">
        <div className="app-container flex flex-col gap-2 py-5 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          <span>
            © {new Date().getFullYear()} LearnUp. Phục vụ mục đích đào tạo.
          </span>
    ...
    <div>
      ...
      <nav className="mt-4 space-y-3" aria-label={title}>
        {links.map(({ label, to }) => (
          <Link
            key={to}
            className="block w-fit text-sm text-slate-600 transition hover:text-blue-700"
            to={to}
          >
            {label}
          </Link>
