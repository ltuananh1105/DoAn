  ...
  const handleSend = async (customText, isRetry = false) => {
    ...
    try {
      ...
      const processEvent = (block) => {
        ...
        if (eventType === "error")
          throw new Error(data.message || "Gemini đang bận, vui lòng thử lại.");
      ...
      setMessages((current) => {
        ...
        return last?.sender === "ai" && !last.text
          ? current.slice(0, -1)
          : current;
      });
    ...
    <div className="fixed bottom-20 right-5 z-50 w-[400px] max-w-[calc(100vw-2rem)] bg-white rounded-xl shadow-xl border border-gray-200 flex flex-col overflow-hidden">
      <div className="px-4 py-3 bg-white border-b text-slate-900 flex justify-between items-center">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="font-bold text-sm">LearnUp Tutor</div>
            <div className="text-[11px] text-slate-500">Trợ lý học tập AI</div>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <button
            ...
            className="p-2 rounded-lg hover:bg-slate-100 text-xs text-slate-500"
          >
            ↻
          </button>
          <button
      ...
      <div
        className="p-4 h-[390px] overflow-y-auto space-y-3 bg-gray-50/70 text-sm"
        aria-live="polite"
      >
        {messages.map((message, index) => (
          ...
          >
            <div
              className={`max-w-[87%] px-3.5 py-3 rounded-2xl whitespace-pre-wrap leading-relaxed shadow-sm ${
                message.sender === "user"
          ...
          <div className="flex justify-start">
            <div className="bg-white px-4 py-3 rounded-2xl border text-gray-500 flex items-center gap-2">
              <span className="flex gap-1">
                <i className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-bounce" />
                <i className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-bounce [animation-delay:120ms]" />
                <i className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-bounce [animation-delay:240ms]" />
              </span>
              Đang suy nghĩ
            </div>
          </div>
      ...
      {messages.length === 1 && (
        <div className="px-3 py-2 bg-white border-t flex gap-1.5 overflow-x-auto text-[11px]">
          {[
            "Sửa giúp tôi câu tiếng Anh",
            "Giải thích thì hiện tại hoàn thành",
            "Luyện hội thoại phỏng vấn",
          ].map((prompt) => (
            <button
              key={prompt}
              onClick={() => handleSend(prompt)}
              className="px-2.5 py-1.5 bg-gray-100 hover:bg-blue-50 hover:text-blue-700 rounded-full whitespace-nowrap text-gray-600"
            >
              {prompt}
            </button>
          ))}
      ...
      <form
        ...
        className="p-3 bg-white border-t flex items-end gap-2"
      >
        <textarea
          ref={inputRef}
          rows="1"
        ...
        <button
          ...
          className="h-10 px-4 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-700 disabled:opacity-50"
        >
          Gửi
        </button>
      </form>
