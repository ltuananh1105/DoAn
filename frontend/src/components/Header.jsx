...
import learnUpLogo from "../assets/logo2.png";
...
export default function Header() {
  ...
  const navClass = ({ isActive }) =>
    `block rounded-md px-3 py-2 text-sm font-medium ${isActive ? "bg-blue-50 text-blue-700" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`;
        ...
        <div className="ml-auto hidden items-center gap-2 md:flex">
          {!user ? (
            <>
              ...
              <Link to="/register" className="ui-button ui-button-primary">
                Đăng ký
              </Link>
            </>
          ) : (
            <>
              {roleLink && !location.pathname.startsWith(roleLink.to) && (
                <Link to={roleLink.to} className="ui-button ui-button-primary">
                  {roleLink.label}
                </Link>
              )}
      ...
      {mobileOpen && (
        <div className="border-t border-slate-200 bg-white px-4 py-3 md:hidden">
          <div className="space-y-1">
            ...
            {user && roleLink && (
              <NavLink to={roleLink.to} className={navClass}>
                {roleLink.label}
              </NavLink>
            )}
            <NavLink to={user ? "/settings" : "/login"} className={navClass}>
              {user ? "Cài đặt tài khoản" : "Đăng nhập"}
            </NavLink>
            {!user && (
            ...
            {user && (
              <button
                onClick={handleLogout}
                className="w-full rounded-md px-3 py-2 text-left text-sm text-red-600"
              >
                Đăng xuất
              </button>
            )}
